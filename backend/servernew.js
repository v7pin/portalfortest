...
// Helper function to generate paths and commands
function getOutputPathAndCommand(template, name, date, gender, internship, period, periodFromDate, periodToDate, grade, city, amount, reportedTo  ) {
  let outputPath;
  let command;

  if (template === 'template1') {
    outputPath = path.join(__dirname, "certificates", "output", `${name}-certificate.jpg`);
    command = `python generate_certificate.py "${name}" "${date}" "${gender}" "${outputPath}"`;
  } else if (template === 'template2') {
    outputPath = path.join(__dirname, "certificates", "output", "experience", `${name}-experience.jpg`);
    command = `python generate_certificate_of_experience.py "${name}" "${internship}" "${period}" "${periodFromDate}" "${periodToDate}" "${grade}" "${gender}" "${outputPath}"`;
  } else if (template === 'template3') {
    outputPath = path.join(__dirname, "certificates", "output", "appreciation", `${name}-appreciation.jpg`);
    command = `python generate_certificate_of_appreciation.py "${name}" "${date}" "${gender}" "${outputPath}"`;
  } else if (template === 'donation') {
    outputPath = path.join(__dirname, "certificates", "output", "donation", `${name}-donation.jpg`);
    command = `python generate_donation_receipt.py "${name}" "${city}" "${amount}" "${outputPath}"`;
  } else if (template === 'offer') {
    outputPath = path.join(__dirname, "certificates", "output", "offer", `${name}-offer.jpg`);
...
app.post("/api/previewCertificate", async (req, res) => {
  const { name, date, gender, template, internship, period, periodFromDate, periodToDate, grade , city , amount, reportedTo} = req.body;
  const { outputPath, command } = getOutputPathAndCommand(template, name, date, gender, internship, period, periodFromDate, periodToDate, grade, city , amount, reportedTo );
...
app.post("/api/sendCertificate", async (req, res) => {
  ...
  if (template === 'template1') {
    ...
    message = `Dear ${name},\n\nCongratulations! We are pleased to offer you an internship at Kshitiksha Foundation. Please find your Offer Letter attached here with this email.\n\nThanks & Best Regards,\nTeam Kshitiksha ❤`;
  }
